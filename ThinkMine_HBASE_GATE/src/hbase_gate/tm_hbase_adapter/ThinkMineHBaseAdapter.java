...
public class ThinkMineHBaseAdapter {
	...
	private static final String[] CF_MIND_MAP = { "mind_map_id", "title",
			"parent_mind_object_id", "mind_objects", "max_rel_distance",
			"max_mind_object_count", "limit_x", "limit_y", "limit_z" };

	private static final String TBL_MIND_OBJECT = "mindobject";
	private static final byte[] TBL_MIND_OBJECT_BYTE = Bytes
			.toBytes(TBL_MIND_OBJECT);
	private static final String[] CF_MIND_OBJECT = { "mind_object_id",
			"child_mind_map_id", "parent_mind_map_id", "shape", "contents",
			"x", "y", "z", "related_mind_objects", "connected_edges" };

	private static final String TBL_EDGE = "edge";
	private static final byte[] TBL_EDGE_BYTE = Bytes.toBytes(TBL_EDGE);
	private static final String[] CF_EDGE = { "first_mind_object",
			"second_mind_object", "edge_type", "edge_type_dependent_info" };

	private static final String TBL_SHAPE = "shape";
	private static final byte[] TBL_SHAPE_BYTE = Bytes.toBytes(TBL_SHAPE);
	private static final String[] CF_SHAPE = { "shape_type",
	...
	private static final String[] CF_CONTENTS = { "contents_type",
			"contents_type_dependent_info" };

	public ThinkMineHBaseAdapter getInstance(
			ThinkMineHbaseConfigContainer aConfigContainer) {
	...
	public void insertNewMindMapInfo(JSONObject aMindMapInfo){
		...
		/* Temporary Code
		String mindMapId = (String)aMindMapInfo.get("MMID");
		String title = (String)aMindMapInfo.get("TT");
		String parentMindObjectId = (String)aMindMapInfo.get("PMOID");
		JSONArray mindObjects = new JSONArray();
		float maxRelDistance = (float)aMindMapInfo.get("MAXRD");
		int maxMindObjectCount = (int)aMindMapInfo.get("MAXOC");
		float LX = (float)aMindMapInfo.get("LX");
		float LY = (float)aMindMapInfo.get("LY");
		float LZ = (float)aMindMapInfo.get("LZ");		
		 Temporary Code*/
	}
	...
	public JSONObject fetchMindMapInfo(String aMindMapId) throws IOException {
		...
		Map<byte[], byte[]> mindObjectsFamiliy = result.getFamilyMap(Bytes
				.toBytes(CF_MIND_MAP[3]));
		...
		for (Map.Entry<byte[], byte[]> entry : mindObjectsFamiliy.entrySet()) {
			...
			Map<byte[], byte[]> relMindObjectsFamiliy = childMindObjectInfo
					.getFamilyMap(tempQuali);
			...
			for (Map.Entry<byte[], byte[]> innerEntry : relMindObjectsFamiliy
				...
				String edgeId = curObjId.compareTo(relMOId) > 0 ? (sb
						.append(curObjId).append(relMOId)).toString() : (sb
						.append(relMOId).append(curObjId)).toString();
				...
				curMORelInfo.add(Bytes.toString(value));
			}
