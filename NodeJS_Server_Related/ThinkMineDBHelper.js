...
var composeMindMapDocument = function(_id, title, parent_mind_object, mind_objects, edges,max_rel_distance, max_mind_object_count, limit_x, limit_y, limit_z){
	var ret = {};
	...
	dbObj.open(function(err,db){
		...
		if (err) {
			...
			edgeCollection = db.collection(EDGE_COLLECTION_NAME);		
							...
							var a = {"_id" : 4, "a" : 3};
	mindMapCollection.insert(a, function(err,result){
	assert.equal(null, err);

		console.log(err);
	});
	edgeCollection.insert(a, function(err,result){
	assert.equal(null, err);
		...
		mindObjectCollection.remove({"_id" : info.MOID},function(err,result){
			...
			mindMapCollection.update({"_id" : parentMindMapId},{"$pull" : {"mind_objects" : {"$ref" : "mindobject" , "$id" : new ObjectID(info.MOID)}}}, function(err, result){
				...
				contentsCollection.remove({"_id" :  contentsId}, function(err,result){
					...
					var count = 0;
					for(var i=0; i<relatedMindObjects.length; i++){
						var edgeId = relatedMindObjects[i].oid > info.MOID ? 
							relatedMindObjects[i].oid+info.MOID : info.MOID+relatedMindObjects[i].oid;
						...
						mindObjectCollection.update({"_id" : relatedMindObjects[i].oid},{"$pull" : {"related_mind_objects" :{"$ref" : "mindobject","$id" : new ObjectID(info.MOID)}}},function (err,result){
							...
							mindMapCollection.update({"_id" : new ObjectID(info.MMID)}, {"$pull" : {"edges" : {"$ref" : "edge","$id" : edgeId}}},function(err,result){
								if(err !=null){
								}
								if(result == null || result == undefined){
								}
								...
								if(count == relatedMindObjects.length){								
									message.retObject = info;
...
this.connectMindObjectAndReply = function(info,requestSocketId) {
	...
	edgeCollection.insert(newEdge,{fullResult : true}, function(err, result){
		if(err != null){
		...
		mindMapCollection.update({"_id" : new ObjectID(info.MMID)},{"$push" : {"edges" : {"$ref" : "edge" , "$id" : newEdgeId}}},function(err,result2){
			if(err!=null){
			}
			
			mindObjectCollection.update({"_id" : new ObjectID(info.MOID)},{"$push" : {"related_mind_objects" : {"$ref" : "mindobject","$id" : new ObjectID(info.TMOID)}}},function(err,result3){
				if(err !=null){				
				}
				mindObjectCollection.update({"_id" : new ObjectID(info.TMOID)},{"$push" : {"related_mind_objects" : {"$ref" : "mindobject","$id" : new ObjectID(info.MOID)}}},function(err,result4){
					if(err !=null){					
					}				
...
this.disconnectMindObjectAndReply = function(info, requestSocketId){
	...
	edgeCollection.remove({"_id" : edgeId}
		,function(err, result){
			...
			mindMapCollection.update({"_id" : new ObjectID(info.MMID)},{"$pull" : {"edges" : {"$ref" : "edge" , "$id" : edgeId}}},function(err,result2){
				if(err!=null){
				}
				
				mindObjectCollection.update({"_id" : new ObjectID(info.MOID)},{"$pull" : {"related_mind_objects" : {"$ref" : "mindobject","$id" : new ObjectID(info.TMOID)}}},function(err,result3){
					if(err != null){
						
					}
					
					mindObjectCollection.update({"_id" : new ObjectID(info.TMOID)},{"$pull" : {"related_mind_objects" : {"$ref" : "mindobject","$id" : new ObjectID(info.MOID)}}},function(err,result4){
						if(err != null){
