...
?>
	<div class="oneByOne_item <?php if ( has_post_thumbnail() ) { ?>has-thumb<?php } else { ?>no-thumb<?php } ?>">
		...
		<div class="text-category" data-animate="fadeInRight"><?php $category = get_the_category(); echo '<a class="category" href="'.get_category_link($category[0]->term_id ).'">'.$category[0]->cat_name.'</a> <a class="category" href="'.get_category_link($category[1]->term_id ).'">'.$category[1]->cat_name.'</a>'; ?></div>
	  	<div class="slider-comments" data-animate="fadeInRight">
			<div class="slider-comments-number"><?php comments_number('0', '1', '%'); ?></div>
			<div class="slider-comments-desc"><?php _e('Comments', 'OneCommunity'); ?></div>
		</div>
	</div>
